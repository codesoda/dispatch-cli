...
- Coordinator-driven lifecycle: `dispatch agent stop`/`restart` mark the worker `stopping` (emitting a `lifecycle` event) *before* signaling the supervisor, so a dying agent's late stop-hook query sees `stopping` and allows the stop instead of fighting the kill. The target may be a worker id or an agent name: a worker id marks only that worker, so stopping one replica leaves its same-named siblings running; a name marks every worker under it.
