...
- Env-driven identity: `dispatch listen`/`ack`/`heartbeat`/`status`/`messages` resolve the worker id from `DISPATCH_WORKER_ID` when `--worker-id` is omitted, and `send`/`team` resolve `--from` the same way. Agents loop with bare commands; no id threading required. Missing identity yields a typed `MissingWorkerIdentity` error.
- `dispatch listen --timeout` and `DISPATCH_LISTEN_TIMEOUT` env var; precedence is `--timeout` flag > `DISPATCH_LISTEN_TIMEOUT` env > per-agent `listen_timeout` > global `listen_timeout` > built-in default (270s). The orchestrator injects `DISPATCH_LISTEN_TIMEOUT` into spawned agents.
- `register --name`/`--role`/`--description` are now optional — when omitted they resolve from `DISPATCH_AGENT_NAME`/`DISPATCH_AGENT_ROLE`/`DISPATCH_AGENT_DESCRIPTION`. Managed agents boot with exactly `dispatch register --for-agent`; the orchestrator injects `DISPATCH_AGENT_DESCRIPTION` and shrinks the boot prompt to a single line.
- Worker control states (`active`/`stopping`/`stopped`), orthogonal to TTL. `Worker.control_state` + `Worker.stopping_since` track an explicit lifecycle; a `stopping` worker finalizes after a lazily-computed drain window (`stopping_drain_secs`, default 10) inside the existing request-driven eviction sweep — no background tick, no auto-revive on claim.
- Coordinator-driven lifecycle: `dispatch agent stop`/`restart` mark the worker `stopping` (emitting a `lifecycle` event) *before* signaling the supervisor, so a dying agent's late stop-hook query sees `stopping` and allows the stop instead of fighting the kill.
- `dispatch result --message-id <id> --status <done|failed|blocked> [--summary ...] [--artifact ...]` — a "super-ack" that rides the `Ack` substrate with additive `status`/`summary`/`artifacts`, reuses ack validation, and emits a `result` event. `--for-agent` prints a terse confirmation.
- `dispatch listen --for-agent` renders a delivered message body verbatim to stdout; on a long-poll timeout it re-queries control state and, while `active`, prints the configured continue-instruction (otherwise emits the neutral JSON timeout).
- Worker-aware stop hook: `dispatch {claude,codex}-hook stop` resolves identity from `DISPATCH_WORKER_ID`, queries the broker for the worker's control state, and **blocks the stop** (injecting the continue-instruction) only while the worker is `active`. No identity, `stopping`/`stopped`, or an unreachable broker → **allow** the stop (failsafe).
- `continue_instruction` config (global + per-agent `[[agents]]` override) with a shipped `DEFAULT_CONTINUE_INSTRUCTION`. Resolved CLI-side in the agent's own process via `ResolvedConfig::continue_instruction_for(agent_name)`, shared by both the stop hook and the listen-timeout renderer so they can't drift.
- New event kinds queryable via `dispatch events`: `prompt` (role-prompt delivery on claim), `lifecycle` (control-state transitions), `stop_decision` (stop-hook block/allow), and `result`. Prompt/packet bodies are logged as a hash + byte size (never the full body) unless `log_prompt_bodies = true` (default false).
- `dispatch` agent skill (`skills/dispatch/`) — a vendor-neutral `SKILL.md` plus `reference/*.md` (register, listen, ack, result, the-listen-loop) documenting the bootstrap and the "don't stop on your own" loop. Symlinked by the user, not embedded.
...
### Changed
- **Breaking:** the `{claude,codex}-hook stop` decision is now worker-aware instead of reachability-based. Previously the hook blocked the stop whenever the broker socket was reachable; now it blocks only when an identified worker is `active`. A reachable broker with no `DISPATCH_WORKER_ID`, or a worker that is `stopping`/`stopped`, allows the stop. This lets a coordinator end an agent's loop by setting its control state, and stops the hook from fighting `dispatch agent stop`.
- **Breaking:** the default `listen` long-poll timeout is 270s (was 30s). Override per-call with `--timeout`/`DISPATCH_LISTEN_TIMEOUT` or in config with `listen_timeout`.
- `dispatch init` template documents the control-plane knobs (`listen_timeout`, `continue_instruction`, `stopping_drain_secs`, `log_prompt_bodies`) and ships `[[agents]]` examples for a supervised worker (`launch = true`) and an interactive coordinator (`launch = false`, `interactive = true`).
